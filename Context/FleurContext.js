...
export function FleurProvider({children, navigation}) {
   ...
   const [messageList, setMessageList] = useState([
    ...
    const [primeFocus, setPrimeFocus] = useState()
      ...
      async function fleurHelper(latestResponse){
        ...
        const response = fetch("https://gpt-all-6oedxiv3iq-ue.a.run.app/gpt/flourish")
   ...
   async function fleurResponseFunc(latestInput, accessToken){
    ...
    try{
    const response = await fetch("https://gpt-all-6oedxiv3iq-ue.a.run.app/gpt/flourish", requestOptions)
    const respJson = await response.json()
        ...
        if(type=="fleur"){
            return(
              <View style={tw`flex-row`}>
                <View id="text-Fleur" style={[tw`${Platform.OS == 'android' ? `bg-slate-600` : `bg-slate-600` } ml-2 justify-center mt-3 items-start rounded-2xl bg-opacity-60 p-2`, {maxWidth:width/1.5}]}>
