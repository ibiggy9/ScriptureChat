...
export default function Fleur({navigation}) {
  ...
  async function heartbeat(){
    try {
      const deviceId1 = await DeviceInfo.getUniqueId();
      const buildNumber = await DeviceInfo.getBuildId()
      const combinedID = deviceId1 + buildNumber
      //const devToken = await DeviceInfo.getDeviceToken()
  
      
      // Creating a unique string
      const response = await axios.post('https://gpt-all-6oedxiv3iq-ue.a.run.app/heartbeat', { device_id: "49F8D5DC-BCFC-43DC-AEE8-2ADC5ACF1A5922D72"});
...
{messageList.length === 1 && !loading && chatMenuShow == false &&
      ...
      <TouchableOpacity onPress={()=> setStartSelection("exercises")} style={[tw`border border-white  rounded-2xl justify-center items-center my-2 px-2`, {width:width/1.5, height:height/20}]}>
        <Text style={[tw`text-white  text-center`, {fontSize:16}]}>Spiritual Growth & Nourishment</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={()=> setStartSelection("scenarios")} style={[tw`border border-white rounded-2xl justify-center items-center my-2`, {width:width/1.5, height:height/20}]}>
        <Text style={[tw`text-white  text-center`, {fontSize:16}]}>Academic & Theological Study</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={()=> setStartSelection("approaches")} style={[tw`border border-white rounded-2xl justify-center items-center my-2`, {width:width/1.5, height:height/20}]}>
        <Text style={[tw`text-white  text-center`, {fontSize:16}]}>Comfort & Support</Text>
      </TouchableOpacity>
      </View>
      </View>
      }
      ...
      <TouchableOpacity style={tw`ml-4 mt-3  mx-10 items-center`} onPress={()=> setStartSelection(null)}>
        <Text style={[tw`text-white font-extrabold text-lg`, {fontSize:16}]}>Go Back</Text>
      </TouchableOpacity>
        </View>
      </>
        }
...
{Platform.OS == 'ios' &&
  ...
  {chatMenuShow == false &&
   ...
   <View id='outer-container' style={[{width:width, maxHeight:height/2}, tw`bg-slate-700 flex-row  bg-opacity-90   ${!isFocused ? `min-h-10 pb-5`: `min-h-12`}`]} >
       
       <View id="inner-container" style={[tw`rounded-2xl `, {width:width}]}>
        <View style={tw`flex-row   my-2 justify-center w-full`}>
       <TouchableOpacity style={tw`mr-1 mb-1 justify-end`} onPress={()=> setChatMenuShow(true)}>
       ...
       numberOfLines={2}
       style={[tw`p-2 ml-2 ${!isFocused && text.length > 20 && ``} justify-end  items-center max-h-30  text-white bg-black rounded-2xl`, { width:width/1.3, fontSize:17}]}
       ...
       <TouchableOpacity style={tw`flex-col ml-3 justify-end mb-1 `} onPress={()=> loading ? null : addMessage("user", text, navigation, accessToken)}>
         {text.length > 650 ? <Text style={tw`font-light  text-slate-200`}>{text.length > 650 && text.length -750}</Text> : null}
