...
export default function PromoCode({ navigation }) {
  ...
  async function offerCode() {
    if (code == 'TESTER2023') {
      ...
      Platform.OS == 'ios'
        ? Linking.openURL(
            `https://apps.apple.com/redeem/?ctx=offercodes&id=6462308226&code=${code}`,
          )
        ...
        {AppState.currentState == 'active' && offerCodeClicked && userLeft && (
          <View style={tw`mt-3`}>
            <Text style={tw`text-white text-center`}>
              If you redeemed a code and were not redirected into the application, press the button
              below.
            </Text>
            <TouchableOpacity
              onPress={() => restorePurchases()}
              style={tw`mt-5 border-2 border-white mx-15 p-4 rounded-2xl`}
            >
